...
class ChoiceWindow(Frame):
	...
	def create_widgets(self):
		...
		self.atk_button = Radiobutton(self, text="Attack", variable=self.button_choice, value="atk", command = get_choice)
		self.atk_button.grid(row = 2, sticky = W)
		self.init_button= Radiobutton(self, text="Initiative", variable=self.button_choice, value="init", command = get_choice)
		self.init_button.grid(row = 3, sticky = W)
		self.skill_button = Radiobutton(self, text="Skill", variable=self.button_choice, value="skil", command = get_choice)
